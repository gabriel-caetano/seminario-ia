...
def split_by_stage(dataset_path='datasets/dataset.csv'):
    ...
    source_file = f'{file_name}_stage234.csv'
    target_file = f'{file_name}_stage5.csv'
    ...
    source_df = df[df[split_column] != 5].copy()
    target_df = df[df[split_column] == 5].copy()
    ...
    source_df.to_csv(f'{file_location}/stage/{source_file}', index=False)
    target_df.to_csv(f'{file_location}/stage/{target_file}', index=False)
    df.to_csv(f'{file_location}/stage/{file_name}_stagefull.csv', index=False)
